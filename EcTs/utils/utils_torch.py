...
def xyz2ics_v2(x,zb,za,zd,eps=1e-7):
    batch_num=x.shape[0]
    max_atom_num=x.shape[1]
    max_bonds=zb.shape[1]
    max_angles=za.shape[1]
    max_dihedrals=zd.shape[1]

    bzids_zb=torch.arange(zb.shape[0]).view(-1,1).tile((1,zb.shape[1])).unsqueeze(-1).to(zb).long()
    zb=torch.concat((bzids_zb,zb),axis=-1).view(-1,3)
    bzids_za=torch.arange(za.shape[0]).view(-1,1).tile((1,za.shape[1])).unsqueeze(-1).to(za).long()
    za=torch.concat((bzids_za,za),axis=-1).view(-1,4)

    bzids_zd=torch.arange(zd.shape[0]).view(-1,1).tile((1,zd.shape[1])).unsqueeze(-1).to(zd).long()
    zd=torch.concat((bzids_zd,zd),axis=-1).view(-1,5)

    zb_it=zb[:,0]*max_atom_num+zb[:,1]
    zb_jt=zb[:,0]*max_atom_num+zb[:,2]

    za_it=za[:,0]*max_atom_num+za[:,1]
    za_jt=za[:,0]*max_atom_num+za[:,2]
    za_kt=za[:,0]*max_atom_num+za[:,3]

    zd_it=zd[:,0]*max_atom_num+zd[:,1]
    zd_jt=zd[:,0]*max_atom_num+zd[:,2]
    zd_kt=zd[:,0]*max_atom_num+zd[:,3]
    zd_lt=zd[:,0]*max_atom_num+zd[:,4]

    x=x.reshape(-1,3)

    zb_x0=torch.index_select(x,dim=0,index=zb_it).view(-1,max_bonds,3)
    zb_x1=torch.index_select(x,dim=0,index=zb_jt).view(-1,max_bonds,3)

    za_x0=torch.index_select(x,dim=0,index=za_it).view(-1,max_angles,3)
    za_x1=torch.index_select(x,dim=0,index=za_jt).view(-1,max_angles,3)
    za_x2=torch.index_select(x,dim=0,index=za_kt).view(-1,max_angles,3)

    zd_x0=torch.index_select(x,dim=0,index=zd_it).view(-1,max_dihedrals,3)
    zd_x1=torch.index_select(x,dim=0,index=zd_jt).view(-1,max_dihedrals,3)
    zd_x2=torch.index_select(x,dim=0,index=zd_kt).view(-1,max_dihedrals,3)
    zd_x3=torch.index_select(x,dim=0,index=zd_lt).view(-1,max_dihedrals,3)

    dist,J_dist=dist_deriv(zb_x0,zb_x1)
    angle,J_angle=angle_deriv(za_x0,za_x1,za_x2)
    dihedral,J_dihedral=torsion_deriv(zd_x0,zd_x1,zd_x2,zd_x3)
    #print (dihedral,torch.isnan(dihedral).any())
    if torch.isnan(dihedral).any():
        print (dihedral,zd_x0,zd_x1,zd_x2,zd_x3)
        raise ValueError('dihedral')
        #exit()
    #dihedral=torsion_deriv(x0,x1,x2,x3)
    return dist,angle,dihedral