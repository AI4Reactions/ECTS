...
def write_xyz(atoms,coords,filepath,title='mol'):
    try:
        mol=IOData(title=title)
        mol.atnums = np.array(atoms)
        mol.atcoords = np.array(coords/0.529)
        with open (filepath,'w') as f:
            iodata.formats.xyz.dump_one(f,mol)
        return 
    except Exception as e:
        print (f'write_xyz failed for {filepath} due to {e}')
        return 
...
def AC2BO(AC, atoms, charge, allow_charged_fragments=True, use_graph=True):
    ...
    for i,(atomicNum,valence) in enumerate(zip(atoms,AC_valence)):
        ...
        if not possible_valence:
            ...
            #sys.exit()
            raise ValueError('Valence of atom',i,'is',valence,'which bigger than allowed max',max(atomic_valence[atomicNum]),'. Stopping')
...
def align_molecules(reference_coords,coords_list):
    aligned_coords_list=[]

    for coords in coords_list:
        P = coords - np.mean(coords, axis=0)
        Q = reference_coords - np.mean(reference_coords, axis=0)
        U = kabsch(P, Q)
        aligned_coords = np.dot(P, U)
        aligned_coords_list.append(aligned_coords)
    return aligned_coords_list

def cal_aligned_average_coords(coords_list):
    ref_coords=coords_list[0]
    aligned_coords_list=align_molecules(ref_coords,coords_list)
    average_coords=np.mean(aligned_coords_list,axis=0)
