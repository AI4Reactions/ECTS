...
class Equiformer_Block(nn.Module):
    @beartype
    def __init__(
        self,  
        token_input_dim = None,
        edge_input_dim = None,
        ...
        **kwargs
        ...
        self.edge_input_dim=token_input_dim 
        self.token_hidden_dim=token_hidden_dim
        self.edge_hidden_dim=edge_hidden_dim

        self.dim=cast_tuple(self.token_hidden_dim,num_degrees)
        self.num_degrees=num_degrees
        self.dim_in = cast_tuple(self.token_hidden_dim, input_degrees)
        self.input_degrees = input_degrees

        # token embedding

        self.token_emb = nn.Sequential(
                nn.Linear(token_input_dim, token_hidden_dim*4),
                nn.SiLU(),
                nn.Linear(token_hidden_dim*4, token_hidden_dim),
            )

        # edges
        self.edge_emb = nn.Sequential(
                nn.Linear(edge_input_dim, edge_hidden_dim*4),
                nn.SiLU(),
                nn.Linear(edge_hidden_dim*4, edge_hidden_dim),
            ) 
        self.has_edges = True
        ...
        edge_dim = (edge_hidden_dim) + (adj_dim if exists(self.adj_emb) else 0)
    ...
    def forward(
        self,
        inputs: Tensor, #(batch, seq, token_input_dim)
        edges: Tensor, #(batch, seq, seq, edge_input_dim)
        coors: Tensor, #(batch, seq, 3)
        mask = None, #(batch, seq)
        adj_mat = None,
        return_pooled = False
    ):
        ...
        assert d == self.token_input_dim, f'feature dimension {d} must be equal to dimension given at init {self.token_input_dim}'
